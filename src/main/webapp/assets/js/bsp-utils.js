(function(globals, factory) {
    ...
    // DOM crawler to check for plugins that were regular AMD modules and have not initialized themselves
    bsp_utils.findPlugins = function(options) {

      var defaults = {
            pluginSelector: '.bsp-plugin',
            pluginLocation: 'modules/'
          },
          $d = $(document),
          settings = $.extend(defaults,options);

      $d.ready(function() {
        $d.find(settings.pluginSelector).each(function() {
          var plugin = {};

          plugin.namespace = $(this).data('pluginNamespace');
          plugin.name = $(this).data('pluginName');

          $(this).attr('data-' + plugin.namespace + '-' + plugin.name,'');

          require([settings.pluginLocation+plugin.name],function(module){
            bsp_utils.plugin(false, plugin.namespace, plugin.name, module);
          });
